...
const infixExprToSuffixExpr = (src) => {
  ...
  let fnArgType = 0; // 1 => , 2 => : 3 => expression
  let fnArgOperator = '';
...
const evalSuffixExpr = (srcStack, formulaMap, cellRender, cellList) => {
  ...
  for (let i = 0; i < srcStack.length; i += 1) {
    ...
    if (expr === '+') {
      ...
      stack.push(formulaMap[name].render(params.reverse()));
    } else if (Array.isArray(expr)) {
      stack.push(expr.map(x => evalSubExpr(x, cellRender)));
    } else {
